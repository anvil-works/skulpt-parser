/* CPython 3.14.3 source-string lexer port experiment.
...
import unicode from "./unicode.json" with { type: "json" };
...
class Scanner {
    ...
    positionByte = 0;
    positionCol = 0;
    constructor(source: string, options: { extraTokens?: boolean; filename?: string }) {
    ...
    make(type: string, a: number | null = null, b: number | null = null): Token {
        ...
        return { type, string, start: [lineno, col], end: [endLine, endCol], line };
    }
    continuation() {
        let c = this.next();
        if (c === 13) c = this.next();
    ...
    verifyEnd(c: number, kind: string) {
        ...
        if (c === 105 && [102, 110, 115].includes(this.bytes[this.cur])) return;
        if (suffix) {
            let j = 0;
            while (j < suffix.length && this.bytes[this.cur + j] === suffix.charCodeAt(j)) j++;
            if (j === suffix.length && !potentialChar(this.bytes[this.cur + j] ?? EOF)) return;
        } // upstream warning-only path
                ...
                if (c === 40 || c === 91 || c === 123) {
                    ...
                    if (this.parens.length) {
                        ...
                        if (
                            ...
                            this.syntax(
                                `closing parenthesis '${String.fromCharCode(c)}' does not match opening parenthesis '${String.fromCharCode(opening.c)}'${opening.line !== this.lineno ? " on line " + opening.line : ""}`
                            );
                        }
                    }
                    if (this.inside) {
    ...
    string(quote: number): Token {
        ...
        while (endSize !== size) {
            ...
            if (c === EOF || (size === 1 && c === 10)) {
                ...
                this.syntax(
                    `unterminated ${size === 3 ? "triple-quoted " : ""}string literal (detected at line ${detected})${size === 1 && escaped ? "; perhaps you escaped the end quote?" : ""}`
                );
            }
            if (c === quote) endSize++;
            else {
    ...
    literal(): Token {
        ...
        while (endSize !== mode.size) {
            ...
            if (c === EOF || (mode.size === 1 && c === 10)) {
                ...
                this.syntax(
                    `unterminated ${mode.size === 3 ? "triple-quoted " : ""}${prefix}-string literal (detected at line ${detected})`
                );
            }
            if (c === mode.quote) {
                endSize++;
