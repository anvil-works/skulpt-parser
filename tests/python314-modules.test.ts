...
// These sources are valid in CPython but explicitly outside the current slice.
for (const source of reference.unsupported) {
    test(`Reject whole module when later statement is unsupported: ${JSON.stringify(source)}`, () => {
