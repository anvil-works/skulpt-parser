...
The slice includes names, numeric and singleton constants, unary/binary/boolean operations, comparisons, conditional expressions, attributes, slicing, tuple/list/set/dictionary displays, unpacking, await, parenthesized assignment expressions, calls, comprehensions, string/bytes literals, f/t-string AST construction, lambdas and yield expressions. All non-diagnostic rules reachable from the pinned grammar’s `eval` entry point are selected. This is grammar coverage, not a claim that every expression edge case has been verified. Module parsing now covers the non-diagnostic statement grammar documented in `docs/python314-modules.md`. Upstream second-pass `invalid_*` diagnostic rules are now enabled; coverage is documented in `docs/python314-diagnostics.md`. Input outside the selected grammar is rejected; this entry point must not replace IDE or Skulpt parsing yet. General parse failures still have a basic syntax error, while tested lexer and numeric errors preserve upstream details.

`tools/generate314/parser.py` selects rules from the checksum-verified CPython 3.14.3 grammar. It traverses the upstream `eval` and `file` dependencies and rejects missing rules, including diagnostic dependencies. An upstream update cannot silently remove newly added module or expression syntax. Actions translate into structural constructors or concrete sequence operations. Unknown semantic calls and ambiguous value-producing actions fail generation; grouped lookahead needs only a success value. Future grammar updates must preserve this completeness check and add CPython-derived conformance fixtures.
...
The expression and module entry points share one standalone bundle. Current sizes and validation totals are recorded in `docs/python314-modules.md`; Unicode names remain included. Public exports remain unchanged, and no speed or memory improvement is claimed. All upstream statement and diagnostic grammar rules are now selected. Broader conformance, semantic validation and consumer integration remain future work.
