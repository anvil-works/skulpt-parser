...
Proposal: public `tokenize` defaults to inspection behavior for IDE/token consumers, while parsing always uses the parser-facing stream internally. Expose an explicit stream option only if a direct consumer needs parser-facing tokens. Follow the selected upstream behavior rather than designing custom lexical recovery.
